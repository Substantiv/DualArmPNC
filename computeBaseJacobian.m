...
b=0.258;
 ...
 mt=0.2;
    ...
    % 输入:
    %   q_arm: 当前关节角度向量
    %   params: 包含动力学参数的结构体，字段包括:
    %       m0, mT, I0, IT - 基座和末端的质量及惯性张量
    %       m_j{i}: 第i段的各关节质量 (cell数组)
    %       I_j{i}: 第i段的各关节惯性张量 (cell数组)
    %       r0: 基座位置 (全局坐标系)
    %       r0_j{i}: 第i段各关节相对于基座的位置 (cell数组)
    %       r0T: 末端相对于基座的位置
    %       J_Tj{i}, J_Rj{i}: 第i段的平动/转动雅可比矩阵 (cell数组)
    %       J_TT, J_RT: 末端的平动/转动雅可比矩阵
    % 输出:
    %   J_bm_v: 式(15)中的基座平动雅可比矩阵 (3x(n1+n2))
    %   J_bm_w: 式(15)中的基座转动雅可比矩阵 (3x(n1+n2))

    % --- 步骤1: 计算总质量 M ---
    m_tott=(m0+m1+m2+m3+m4+m5+m6+mt);
    
    % --- 步骤2: 计算质心位置 r0g ---
    % 假设rg是全局质心，需通过各部分质心计算
    % 此处简化为各部分质心加权平均
